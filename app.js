...
const heroTotalProximos = document.getElementById('heroTotalProximos');
const totalAmanha = document.getElementById('totalAmanha');
const agendamentosAmanhaLista = document.getElementById('agendamentosAmanhaLista');
...
function montarMensagemWhatsApp(item) {
  return `Olá, ${item.pacienteNome}! Passando para lembrar do seu agendamento na UME Estética Avançada.

📅 Data: ${formatarDataBR(item.data)}
⏰ Horário: ${String(item.hora || '').slice(0, 5)}
💆 Procedimento: ${item.procedimentoNome}
👩‍⚕️ Profissional: ${item.profissional}

Se precisar remarcar, me avise por aqui.`;
}

function obterDadosWhatsAppAgendamento(item) {
  const telefone = normalizarTelefoneWhatsApp(item.pacienteTelefone || obterPacientePorId(item.pacienteId)?.telefone || '');
  if (!telefone) return null;
  const texto = encodeURIComponent(montarMensagemWhatsApp(item));
  return `https://wa.me/${telefone}?text=${texto}`;
}

function lembreteEnviado(item) {
  return !!item.lembreteWhatsAppEnviadoEm;
}

function marcarLembreteEnviado(id) {
  const idx = agendamentos.findIndex(a => a.id === id);
  if (idx === -1) return;
  agendamentos[idx].lembreteWhatsAppEnviadoEm = new Date().toISOString();
  salvar();
}
...
function enviarWhatsAppAgendamento(id) {
  ...
  if (!item) return;
  const link = obterDadosWhatsAppAgendamento(item);
  if (!link) return mostrarToast('Cadastre o telefone do paciente para usar o WhatsApp.');
  window.open(link, '_blank');
  marcarLembreteEnviado(id);
  renderAgenda();
  mostrarToast('Lembrete marcado como enviado.');
}
window.enviarWhatsAppAgendamento = enviarWhatsAppAgendamento;

function obterAgendamentosAmanha() {
  const amanha = dataAmanhaISO();
  return agendamentos
    .filter(a => a.data === amanha && a.status !== 'cancelou' && a.status !== 'remarcou')
    .sort((a, b) => `${a.data} ${a.hora}`.localeCompare(`${b.data} ${b.hora}`));
}

function abrirLembretesAmanha() {
  const lista = obterAgendamentosAmanha().filter(item => !!obterDadosWhatsAppAgendamento(item) && !lembreteEnviado(item));
  if (!lista.length) return mostrarToast('Não há lembretes pendentes válidos para amanhã.');
  ...
  lista.forEach((item, index) => {
    ...
    setTimeout(() => {
      ...
      if (win) {
        opened += 1;
        marcarLembreteEnviado(item.id);
      }
      if (index === lista.length - 1) {
        setTimeout(() => {
          renderAgenda();
          if (opened < lista.length) {
            mostrarToast('Alguns lembretes podem ter sido bloqueados pelo navegador. Se necessário, use os botões individuais.');
          } else {
            mostrarToast(`Foram abertos ${opened} lembrete(s) de amanhã.`);
          }
        }, 250);
      }
    }, index * 350);
...
window.abrirLembretesAmanha = abrirLembretesAmanha;
...
function atualizarMetricas() {
  ...
  const amanha = obterAgendamentosAmanha();
  totalHoje.textContent = agHoje.length;
  if (summaryTotalHoje) summaryTotalHoje.textContent = agHoje.length;
  if (totalProximos) totalProximos.textContent = proximos.length;
  if (heroTotalProximos) heroTotalProximos.textContent = proximos.length;
  if (totalAmanha) totalAmanha.textContent = amanha.length;
...
function renderAgendamentosAmanha() {
  const lista = obterAgendamentosAmanha();
  if (!agendamentosAmanhaLista) return;
  if (!lista.length) {
    agendamentosAmanhaLista.innerHTML = '<div class="tomorrow-item empty-state-card"><div><h3>Nenhum agendamento para amanhã</h3><p>Quando houver, os lembretes aparecerão aqui.</p></div></div>';
    return;
  }

  agendamentosAmanhaLista.innerHTML = lista.map(item => {
    const link = obterDadosWhatsAppAgendamento(item);
    let actionHTML = '<button class="btn ghost small-action" type="button" disabled>Sem WhatsApp</button>';
    if (link && lembreteEnviado(item)) {
      actionHTML = '<span class="badge-reminder">Lembrete enviado</span>';
    } else if (link) {
      actionHTML = `<button class="btn whatsapp small-action" onclick="enviarWhatsAppAgendamento(${item.id})" type="button">Enviar lembrete</button>`;
    }
    return `
      <article class="tomorrow-item">
        <div class="top">
          <div>
            <h4>${escapeHTML(item.pacienteNome)}</h4>
            <small>${formatarDataBR(item.data)} às ${escapeHTML(String(item.hora || '').slice(0, 5))}</small>
          </div>
          <span class="status-pill ${statusPillClass(item.status)}">${escapeHTML(item.status || 'agendado')}</span>
        </div>
        <div class="meta">
          <span><strong>Procedimento:</strong> ${escapeHTML(item.procedimentoNome)}</span>
          <span><strong>Profissional:</strong> ${escapeHTML(item.profissional)}</span>
          <span><strong>Telefone:</strong> ${escapeHTML(item.pacienteTelefone || obterPacientePorId(item.pacienteId)?.telefone || '-')}</span>
        </div>
        <div class="foot">
          ${actionHTML}
        </div>
...
function renderAgenda() {
  ...
  agendaLista.innerHTML = lista.map(item => {
    const link = obterDadosWhatsAppAgendamento(item);
    const isTomorrow = item.data === amanha;
    let whatsArea = '';
    if (link && lembreteEnviado(item)) {
      whatsArea = '<span class="badge-reminder">Lembrete enviado</span>';
    } else if (link) {
      const whatsLabel = isTomorrow ? 'Enviar lembrete' : 'WhatsApp';
      whatsArea = `<button class="btn whatsapp small-action" onclick="enviarWhatsAppAgendamento(${item.id})" type="button">${whatsLabel}</button>`;
    }
...
function inicializarModais() {
  ...
  document.querySelectorAll('.modal').forEach(modal => modal.addEventListener('click', (e) => {
    if (e.target === modal) {
      ...
      const aberto = [...document.querySelectorAll('.modal')].some(m => !m.classList.contains('hidden'));
      document.body.classList.toggle('modal-open', aberto);
    }
  }));

  document.getElementById('btnNovoPaciente').addEventListener('click', () => { limparModalPaciente(); abrirModal('modalPaciente'); });
  ...
  document.getElementById('btnEnviarLembretesAmanha').addEventListener('click', abrirLembretesAmanha);
...
function inicializarEventosFormulario() {
  document.getElementById('salvarPaciente').addEventListener('click', () => {
    ...
    if (pacienteEditandoId) {
      ...
      agendamentos = agendamentos.map(a => (a.pacienteId === pacienteEditandoId || a.pacienteNome === nomeAnterior)
        ? { ...a, pacienteId: pacienteEditandoId, pacienteNome: nome, pacienteTelefone: payload.telefone }
        : a);
  ...
  document.getElementById('salvarAgendamento').addEventListener('click', () => {
    ...
    if (conflitos.length) {
      return mostrarToast('Horário já agendado, selecione outra data/horário');
    }
...
function inicializarHintsPWA() {
  if (dismissInstallHint) dismissInstallHint.addEventListener('click', () => { localStorage.setItem(STORAGE_KEYS.installHintDismissed, '1'); esconderInstallHint(); });
  if (installHintOk) installHintOk.addEventListener('click', () => { localStorage.setItem(STORAGE_KEYS.installHintDismissed, '1'); esconderInstallHint(); });
...
function seedInicial() {
  if (!pacientes.length && !procedimentos.length && !agendamentos.length) {
    ...
    agendamentos.push({ id: 1, pacienteId: 1, pacienteNome: paciente.nome, pacienteTelefone: paciente.telefone, procedimentoId: 1, procedimentoNome: procedimento.nome, procedimentoDescricao: procedimento.descricao, procedimentoValor: procedimento.valor, data: dataHojeISO(), hora: '14:00', profissional: 'Dra. Ana', descricaoCurta: 'Sessão facial rápida', recorrencia: 'nenhuma', totalPacote: 1, ordemPacote: 1, pacoteId: null, status: 'agendado', lembreteWhatsAppEnviadoEm: null });
    salvar();
  }
}
