...
function enviarWhatsAppAgendamento(id) {
  ...
  if (!telefone) return mostrarToast('Cadastre o telefone do paciente para usar o WhatsApp.');
  const txt = encodeURIComponent(montarMensagemWhatsApp(item));
  window.open(`https://wa.me/${telefone}?text=${txt}`, '_blank');
}
...
function renderAgenda() {
  ...
  agendaLista.innerHTML = lista.map(item => {
    ...
    const whatsBtn = telefone
      ? `<button class="btn whatsapp small-action" onclick="enviarWhatsAppAgendamento(${item.id})" type="button">${whatsLabel}</button>`
      : '';
    return `
